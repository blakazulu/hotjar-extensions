...
// Hide score display on mobile
if (isMobile) {
  const scoreDisplay = document.getElementById("scoreDisplay");
  if (scoreDisplay) {
    scoreDisplay.style.display = "none";
  }
  console.log("Mobile device detected - simplified mode enabled");
} else {
  console.log("Desktop device detected - full game mode enabled");
}
...
async function initGame() {
  try {
    ...
    // Mobile: 30 particles only, Desktop: 120 particles
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      // Store particle behavior data - slower movement on mobile
      particleData.push({
        sprite: sprite,
        speedX: (Math.random() - 0.5) * (isMobile ? 0.2 : 0.4),
        speedY: (Math.random() - 0.5) * (isMobile ? 0.2 : 0.4),
        originalAlpha: sprite.alpha,
      });
    }

    console.log(`Created ${particleCount} background particles (${isMobile ? "mobile" : "desktop"} mode)`);

    // Connection lines between nearby particles (desktop only)
    const connectionsGraphics = isMobile ? null : new PIXI.Graphics();
    if (connectionsGraphics) {
      gameApp.stage.addChild(connectionsGraphics);
    }

    // ===== SHIELD SYSTEM (DESKTOP ONLY) =====
    const shieldGraphics = isMobile ? null : new PIXI.Graphics();
    if (shieldGraphics) {
    ...
    // Shield trail effect (desktop only)
    const trailPoints = [];
    const maxTrailPoints = 20;

    // ===== THREAT SYSTEM (SPRITE-BASED, DESKTOP ONLY) =====
    const threats = [];
    const maxThreats = isMobile ? 0 : 30;
    ...
    gameApp.ticker.add((ticker) => {
      ...
      // Desktop: Update shield pulse and position
      if (!isMobile) {
      ...
      for (let i = 0; i < particleData.length; i++) {
        ...
        // Desktop only: Subtle attraction to mouse cursor
        if (!isMobile) {
      ...
      // ===== DRAW PARTICLE CONNECTION NETWORK (v8 CORRECTED API, DESKTOP ONLY) =====
      if (connectionsGraphics) {
      ...
      // Desktop only: Update shield trail and draw shield
      if (!isMobile) {
        // Update shield trail
    ...
    window.addEventListener("beforeunload", () => {
      ...
      // Destroy graphics
      if (connectionsGraphics) connectionsGraphics.destroy();
      if (shieldGraphics) shieldGraphics.destroy();
    ...
    if (isMobile) {
      console.log("%c📱 MOBILE MODE - AMBIENT PARTICLES", "font-size: 20px; color: #f97316; font-weight: bold;");
      console.log("%cSimplified mode: 30 ambient particles for performance", "font-size: 14px; color: #64748b;");
      console.log("%cPixiJS v8 optimizations:", "font-size: 14px; color: #8b5cf6;");
      console.log("%c- ParticleContainer batch rendering (30 particles → 1 draw call)", "font-size: 12px; color: #10b981;");
      console.log("%c- Mobile-optimized rendering", "font-size: 12px; color: #10b981;");
    } else {
